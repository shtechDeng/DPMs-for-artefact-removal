...
class CondationModel(nn.Module):
    def __init__(self, config):
        ...
        self.model_block = nn.ModuleList()
    ...
    def forward(self, x, temb):
        for model in self.model_block:
            x = nonlinearity(model(x + self.temb_proj(nonlinearity(temb))[:, :, None, None, None]))
...
class Model(nn.Module):
    ...
    def forward(self, x, t):
        ...
        # c1 = self.condation_model(c1, temb) + self.temb_proj(nonlinearity(temb))[:, :, None, None, None]
